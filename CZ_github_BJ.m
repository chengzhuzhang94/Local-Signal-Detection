...
%% Use a fixed Triangulation. This one
...
% Data Visualization of All data points and TRI
...
% Downsize the data, only valid points falling into TRI are left
[B, valid_id] = CZ_SPL_est(df(:, 3),df(:, 4),vx,vy,TRI,v1,v2,v3,nt,nc,nv,d); 
B = B(valid_id, :); df = df(valid_id,:); 
% only 277,410 out of 293,963 data points are left
...
% One-time run to get the optimized lambda value
...
% SSE_SCAD = sum((totalPrice_log_std(train_id)-mat_Z * p_b_hat).^2)%3.3690e+04
% n*log(SSE_SCAD/n)+2*sum(p_b_hat~=0)%-4.2571e+05
% 
...
% SSE_SCAD = sum((totalPrice_log_std(test_id)-test_mat_Z * p_b_hat).^2)%7.7853e+03
% n*log(SSE_SCAD/n)+2*sum(p_b_hat~=0) %-7.5533e+05

%% Cross-validation
n_total = (length(train_id)+length(test_id)) % 277,410
