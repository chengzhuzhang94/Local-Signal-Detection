function [records, lambda_records] = CZ_bootstrap_logic_nested(seed, TRI, pv, vx, vy, n, mat_Z, Z, b_hat, p_b_hat, nt, nc, nv, d, v1, v2, v3, e1, e2, e3, ie1, m, kLoopTime, bootstrap_opt, lambda_opt)
...
if bootstrap_opt==1
    for loop=1:kLoopTime
        ...
        % Use the first bootstrap sample to get the optimized_lambda value
        if exist('optimized_lambda', 'var') == 0
            ...
            [temp_min, temp_index] = min(bic(:, 1)); optimized_lambda = lam_vec(temp_index); % Initialize the optimized_lambda
        end
        
        if lambda_opt == 1
            % This option directly applies the optimized_lambda
