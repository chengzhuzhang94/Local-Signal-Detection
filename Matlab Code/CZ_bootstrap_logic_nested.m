...
% Purpose:
% This function implements inner Bootstrap fitting of MCR for logistic regression

% Arguments:
% seed: random seed
% lam_vec: a list of candidate lambda values (SCAD penalty hyperparameter), we turn to use the best lambda out of this candidate list
% mat_Z: original design matrix
% Z: original response variable
% b_hat: original UNPEN estimator
% p_b_hat: original SCAD estimator
% n: sample size
% kLoopTime: the number of inner loop that used to calculate one pair of LPR and UPR
% bootstrap_opt: indicator of which type of Bootstrap is used. The default value is 1 (case resampling)
% lambda_opt: indicator of whether using fixed hyperparmeter "lambda" or pick the new optimized one from lam_vec. "1" means use fixed and "0" means re-fitting to get the optimized
% optimized_lambda: the optimized lambda value from the raw dataset
    ...
    for loop=1:kLoopTime
        for vcf_no=1:m
            ...
            if optimized_lambda(vcf_no) ~= 0 && lambda_opt == 1
                ...
                nlam = 12; a = 3.7;threshold = 10 ^ (-3); lam_vec = linspace(0.25, 0.8, nlam);
                bic = zeros(nlam, 1); converged_or_not = zeros(nlam, 1);
                for q = 1:nlam
                    [p_b_hat, dist_logical, probs, first_deri_l, second_deri_l, Dgn, accuracy_record] = update_p_b_hat_logistic_NoC(...
